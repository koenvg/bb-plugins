// bb-plugin-tasks — a BB plugin backend entry.
//
...
// surfaces: the Example todos page (app.tsx, over RPC), the `bb tasks` CLI
...
export default async function plugin(bb: BbPluginApi) {
  ...
  // `bb plugin config tasks`. Add `secret: true` for values like API keys.
  ...
  // The `bb tasks` command: what agents (and you) use from a shell. Parsing
  ...
  const usage = [
    "Usage:",
    "  bb tasks list [--json]",
    "  bb tasks add <title> [--json]",
    "  bb tasks done <todo-id> [--json]",
    "  bb tasks undo <todo-id> [--json]",
    "  bb tasks remove <todo-id> [--json]",
  ].join("\n");
  ...
  bb.cli.register({
    name: "tasks",
    summary: "Manage the Tasks plugin's example todo list",
    commands: [
      { name: "list", summary: "List todos", usage: "bb tasks list [--json]" },
      {
        name: "add",
        summary: "Add a todo",
        usage: "bb tasks add <title> [--json]",
      },
      {
        name: "done",
        summary: "Mark a todo done",
        usage: "bb tasks done <todo-id> [--json]",
      },
      {
        name: "undo",
        summary: "Mark a todo not done",
        usage: "bb tasks undo <todo-id> [--json]",
      },
      {
        name: "remove",
        summary: "Remove a todo",
        usage: "bb tasks remove <todo-id> [--json]",
      },
    ],
    async run(argv) {
      ...
      const notFound = (missingId: string) => ({
        exitCode: 1,
        stderr: `No todo with id ${missingId}. Run "bb tasks list" to see ids.`,
      });
      const todoId = args[0];
      switch (command) {
        case undefined:
